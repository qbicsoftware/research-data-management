...
!!! info
    If you are not a member of the University of Tübingen, please [request access as a data submitter](upload_server_access.md) 
...
- A connection to the University Of Tübingen network (
  ...
  or [data submitter account](upload_server_access.md)
...
=== "External data submitter"
    You can log in with your username and key file. Enter your username
    into the `User` field and select your private key file by pressing the browse button.
...
=== "External Data Submitter"
    ``` bash
    sftp -i <your private key file> <upload server username>@upload.qbic.uni-tuebingen.de
    ```

=== "University Tübingen Member"
    ``` bash
    sftp <university tuebingen user namer>@upload.qbic.uni-tuebingen.de
    ```
